...
public class BshMonstor : MonoBehaviour
{


    public LayerMask monsterlayer;//可以碰撞弹射的球
    public float bashRadius;//检测可弹射的范围半径
    public float bashForce;//玩家弹射力
    public float monsterForce;//怪物弹射力
    ...
    private void Bash()
    {
        ...
        {
            bashPoint = Physics2D.OverlapCircleAll(transform.position, bashRadius, monsterlayer);//获得碰撞点数组
        ...
        else if (Input.GetMouseButtonUp(1) && canBash)
        {
            ...
            foreach (Collider2D _bashPoint in bashPoint)
            {
                ...
                _rb.AddForce(-1 * bashDirection * monsterForce, ForceMode2D.Impulse);//弹射目标
