...
public class BushGround : MonoBehaviour
{


    public LayerMask groundlayer;//可以碰撞弹射的墙
    ...
    private void Bash()
    {
        ...
        if (Input.GetMouseButtonDown(1))
        {
            ...
            bashPoint = Physics2D.OverlapCircleAll(transform.position, bashRadius, groundlayer);//获得碰撞点数组
            if (bashPoint.Length != 0)
            {
                anim.SetTrigger("Attack");

                bashArrow.gameObject.SetActive(true);//显示箭头
                bashArrow.position = transform.position;//改变箭头位置为当前弹射点
        ...
        else if (Input.GetMouseButtonUp(1) && canBash)
        {
            ...
            //修改x,和y方向的弹力系数大小
            bashDirection.y = bashDirection.y * 2.0f;
